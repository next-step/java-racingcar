...
```
#### `car` 객체 생성하기
- `potision`의 초기값을 지정하지 않고 `car` 생성
```java
public static void main(String[] args) {
...
- 앞으로 전진 여부를 `car`객체가 선택하도록 클래스 구현하기
...
#### 객체를 의인화하기
- 객체를 수동적으로 바라보지 말고, 능동적으로 행동(판단, 판별)한다고 생각해보기
- `car`를 의인화 해보기 ex) 앞으로 갈지 말지, 초기값을 미리 가지기
