...
public class ConsoleApplication {
    ...
    public static void main(String[] args) {
        ...
        List<String> names = InputView.getStringList("경주할 자동차 이름을 입력하세요(이름은 쉼표(" + SPLIT_REGEX + ")를 기준으로 구분) ", SPLIT_REGEX, scanner);
        ...
        for (int i = 0; i < tryCount; i++) {
            for (Car car : cars) {
                racingGame.run(car);
                System.out.println(car.getName() + " : " + OutputView.generateRaceResultString(car));
            }
            System.out.println();
        }
