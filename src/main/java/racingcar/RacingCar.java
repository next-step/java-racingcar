...
public class RacingCar {
    ...
    public static List<Car> makeCarList(String carNames) {
        List<String> carNameList = Arrays.stream(makeCarNameList(carNames)).collect(Collectors.toList());
    ...
    public static String[] makeCarNameList(String carNames) {
        String[] carNameArr = carNames.split(",");
        Arrays.stream(carNameArr).forEach(carName -> {
            if(carName.length()>nameLength) throw new StringIndexOutOfBoundsException("자동차 이름은 5자를 초과할 수 없습니다.");
        });
