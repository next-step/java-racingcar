...
* [x] 자동차가 전진할 수 있는 조건이 필요하다.
* [x] 해당 조건에 따른 결과로 자동차는 이동이 가능하다.
* [x] 숫자가 4(이동조건)보다 작다면 자동차는 전진하지 않는다.
* [x] 숫자가 4(이동조건)이상 이라면 자동차는 전진한다.
* [x] 입력받은 자동차 이름을 ,로 구분하여 자동차를 생성한다.
* [X] 자동차의 이름은 5글자를 초과할 수 없다.
* [X] 자동차의 이름이 0글자라면 경주는 개최하지 않는다.
* [X] 자동차가 전진하면 자동차의 이름을 함께 출력한다. 
* [X] 자동차의 이름을 구분할 수 없으면 경주는 개최하지 않는다.
* [X] 게임을 완료 후 우승자를 알려준다.
* [X] 우승자는 복수 일 수 있다.
