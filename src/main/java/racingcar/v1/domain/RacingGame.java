...
import racingcar.v1.repository.RacingGameRepository;
...
public class RacingGame {
    ...
    public List<String> getWinners(int tryNumber) {
        ...
        int maxPosition = findMaxPosition(recordList);

        List<String> winnerList = new ArrayList<>();
        findWinnerList(winnerList, recordList, maxPosition);

        return winnerList;
    }

    private int findMaxPosition(List<RacingRecord> recordList) {
        int maxPosition = 0;
    ...
    private void findWinnerList(List<String> winnerList, List<RacingRecord> recordList, int maxPosition) {
        for (RacingRecord racingRecord : recordList) {
