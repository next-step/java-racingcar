...
힌트
1. 규칙 1: 한 메서드에 오직 한 단계의 들여쓰기(indent)만 한다.
   - indent가 2이상인 메소드의 경우 메소드를 분리하면 indent를 줄일 수 있다.
   - else를 사용하지 않아 indent를 줄일 수 있다.
2. 자동차 이름을 쉼표(,)를 기준으로 분리하려면 String 클래스의 split(",") 메소드를 활용한다.
   String[] names = inputName.split(",");
3. 사용자가 입력한 이름의 숫자 만큼 자동차 대수를 생성한다.
4. 자동차는 자동차 이름과 위치 정보를 가지는 Car 객체를 추가해 구현한다
