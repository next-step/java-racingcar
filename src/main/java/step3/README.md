...
#### 궁금한 점
(2차 PR 요청 - 궁금한 점 추가)
InputView에서 각 메서드에 return값을 추가하다보니 생성자에서 두 메서드를 호출할 필요가 없다고 생각 되어 삭제 및  static 메서드로 선안
-> 이 때 스캐너를 한번에 인스턴스화 하던 부분을 각 메서드 안으로 추가했는데 두 메서드에 중복된 코드가 발생합니다.
 scanner = new Scanner(System.in);   
 이런 부분은 어떻게 구성하는게 좋을까요? 애초에 static메서드로 하지 않고 인스턴스 생성 후 메서드를 호출하더라도
 생성자에서  scanner = new Scanner(System.in); 를 추가하는게 괜찮은 방법인가요? 
