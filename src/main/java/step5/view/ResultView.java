...
public class ResultView {
  ...
  public void printResult(Cars cars) {
    ...
    cars.getCars().forEach(c -> {
      stringBuilder.append(makeIndividualCarsScore(c.showCarName(), c.getPosition().getPosition()));
      stringBuilder.append(RESULT_VIEW_LINE);
    });
    System.out.println(stringBuilder.toString());
  }

  public void afterRunScript(List<Car> champs) {
    StringBuilder stringBuilder = new StringBuilder();
    if (champs.size() == 0) {
    ...
    for (int i = 0; i < champs.size() - 1; i++) {
      stringBuilder.append(champs.get(i).showCarName()).append(VARIABLE_NAME_SPLIT).append(" ");
    }
    stringBuilder.append(champs.get(champs.size() - 1).showCarName());
