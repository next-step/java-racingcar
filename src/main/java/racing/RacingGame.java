...
public class RacingGame {
    ...
    public static void start() {
        ...
        inputRound(rule, cars);
    }
    public static void inputRound(Rule rule, Car[] cars) {
        System.out.println("시도할 횟수는 몇 회인가요?.\n");
        Scanner roundInput = new Scanner(System.in);
        int round = roundInput.nextInt();
        rule.setRound(round);
        System.out.println("실행 결과");
        roundMatch(rule, cars);
        rule.finalWinner();
    }
    public static void roundMatch(Rule rule, Car[] cars) {
        for (int roundIdx = 0; roundIdx < rule.round && rule.isGame; roundIdx++) {
            Run(rule, cars);
        }
    }
    public static void Run(Rule rule, Car[] cars) {
        for (int carsIdx = 0; carsIdx <= cars.length - 1; carsIdx++) {
            System.out.print(cars[carsIdx].getName() + " : ");
            rule.racingPrint(cars[carsIdx].getSpeed());
            System.out.println();
            rule.setWinnerCount(cars[carsIdx].getName(), cars[carsIdx].count);
        }
    }
