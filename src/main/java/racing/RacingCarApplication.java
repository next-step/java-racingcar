...
public class RacingCarApplication {
  ...
  public static void main(String[] args) {
    ...
    CarManager carManager = new CarManager(StringToArray(carNamesRaw), new RandomGenerator());

    System.out.println("실행 결과");

    for (int i = 0; i < roundCount; i++) {
      carManager.simulateRace();
      ResultView.printRaceResult(carManager.getCars());
    }

    List<String> maxPosition = carManager.findWinners();
