...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        final String[] cars = input.inputCSV("경주할 자동차 이름을 입력하세요(이름은 쉼표(,)를 기준으로 구분).", Car.NAME_LIMIT, DELIMITER);
        final int turns = input.inputPositiveNumber("시도할 회수는 몇 회 인가요?");

        final ResultView result = new ResultView(turns);

        final Race race = Race.of(cars, turns, RandomMovePolicy.getInstance());
        while (!race.isEnded()) {
            race.process();
