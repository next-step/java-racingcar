package resource;

public class StringResources {

    // Error msg
    public static final String ERR_INCORRECT_EQUATION = "Please enter the correct equation";
    public static final String ERR_INCORRECT_OPERATOR = "Please enter the correct operator.";
    public static final String ERR_DIVIDED_BY_ZERO = "It cannot be divided by zero.";
    public static final String ERR_INVALID_CAR_DATA = "입력한 자동차 대수와 이름의 수가 일치하지 않습니다.";
    public static final String ERR_INVALID_CAR_NAMES = "자동차 이름이 입력되지 않았습니다.";
    public static final String ERR_LONG_NAME = "이름은 5자를 초과할 수 없습니다.";

    // Text Resources
    public static final String INPUT_CAR_NAME = "경주할 자동차 이름을 입력하세요(이름은 쉼표(,)를 기준으로 구분).";
    public static final String INPUT_NUMBER_OF_CARS = "자동차 대수는 몇 대 인가요?";
    public static final String INPUT_TRY_COUNT = "시도할 수는 몇 회 인가요?";
    public static final String GAME_RESULT = "실행 결과";
    public static final String GAME_RESULT_WINNERS = "가 최종 우승했습니다.";
}
