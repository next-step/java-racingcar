...
public class InputValidatorTest {
    ...
    void pass_invalid_match_count_value_illegal_argument_exception() {
        assertThatThrownBy(() -> InputValidator.validateMatchCount(-1))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("입력한 값이 0 미만 일 수 없습니다.");
    }

    @DisplayName("5단계 - 자동차 경주(리팩토링) - 레이싱 Match Count 에 0 이상의 값을 입력한 경우 예외가 발생하지 않는지 확인")
    @Test
    void pass_valid_match_count_value_illegal_argument_exception() {
        assertThatCode(() -> InputValidator.validateMatchCount(0))
                .doesNotThrowAnyException();
    }
}
