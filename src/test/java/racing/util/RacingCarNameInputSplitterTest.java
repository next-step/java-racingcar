...
class RacingCarNameInputSplitterTest {
  ...
  void splitCarNames_givenCommaSeparatedString_returnsListOfCarNames() {
    List<RacingCarName> result = RacingCarNameInputSplitter.split("pobi,crong,honux");
  ...
  void throwRuntimeException_whenLessThanTwoCarNamesProvided() {
    assertThrows(RuntimeException.class, () -> RacingCarNameInputSplitter.split("pobi"));
  }

  @DisplayName("차의 이름들을 받았을 때 이름 중 하나라도 빈 문자열이면 RuntimeException을 던진다.")
  @Test
  void throwRuntimeException_whenAnyCarNameIsEmpty() {
    assertThrows(RuntimeException.class, () -> RacingCarNameInputSplitter.split("pobi,,crong"));
  }

  @DisplayName("차의 이름들을 받았을 때 null이면 RuntimeException을 던진다.")
  @Test
  void throwRuntimeException_whenCarNamesNull() {
    assertThrows(RuntimeException.class, () -> RacingCarNameInputSplitter.split(null));
  }
}
