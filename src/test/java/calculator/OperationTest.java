...
class OperationTest {
	...
	void shouldEqualsSymbolCharAndSymbolName(String symbol, String symbolName) {
		assertThat(Operation.valueOf(symbolName).getSymbol()).isEqualTo(symbol);
	}
	...
	void shouldFindStringSymbolFromOperation(String symbol) {
		assertThat(Arrays.stream(Operation.values()).anyMatch(v -> v.getSymbol().equals(symbol))).isTrue();
	}
