...
public class NumberTest {
    ...
    void 생성테스트_02(final String number) {
        assertThatThrownBy(() -> Number.from(number)).isInstanceOf(NumberException.class).hasMessageContaining(NOT_ALLOWED_NEGATIVE.getMessage());
    }
    ...
    void 생성테스트_05(final String text) {
        assertThatThrownBy(() -> Number.from(text)).isInstanceOf(NumberException.class).hasMessageContaining(NON_NUMERICAL.getMessage());
    }
    ...
    void add() {
        // given
        final var augend = Number.from("10");
        final var addend = Number.from("5");

        // when
        final var sum = augend.add(addend);

        // then
        assertThat(sum).isEqualTo(Number.from("15"));
    }
