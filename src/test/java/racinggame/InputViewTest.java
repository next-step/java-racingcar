...
class InputViewTest {
    ...
    @DisplayName("실패 - 자동차 대수 입력 시 숫자 형식이 아닌 문자를 입력할 경우 예외가 발생한다.")
    void fail_input_mismatch_number_car_size(String text) {
        InputView inputView = new InputView(scanner(text));
        Assertions.assertThatThrownBy(inputView::carSize)
