...
class PositiveTest {
  ...
  @Test
  void 생성자_양수() {
    assertThat(new Positive(1).value()).isEqualTo(1);
  }
  ...
  @Test
  void from_양수_문자열() {
    assertThat(Positive.from("1").value()).isEqualTo(1);
  }

  @Test
  void from_음수_문자열() {
    assertThatThrownBy(() -> Positive.from("-1"))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("음수는 입력할 수 없습니다: -1");
  }
  ...
  @Test
  void from_null() {
    assertThatThrownBy(() -> Positive.from(null))
  ...
  void from_공백_또는_빈_문자열(String input) {
    assertThatThrownBy(() -> Positive.from(input))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void from_숫자가_아닌_문자열() {
    assertThatThrownBy(() -> Positive.from("a"))
        .isInstanceOf(IllegalArgumentException.class)
