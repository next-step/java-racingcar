...
class CarNamesTest {
    ...
    @DisplayName("자동차 이름 길이가 유효하지 않다면 IllegalArgumentException 예외를 발생시킨다.")
    @ParameterizedTest
    @ValueSource(strings = { "", "pobi, honux,invalid"})
    void of_invalid_car_name(String invalidCarNames) {
        assertThatThrownBy(() -> {
            CarNames.of(invalidCarNames);
        }).isInstanceOf(IllegalArgumentException.class).hasMessageMatching("The car name is at least 1 characters and up to 5 characters.");
    }
}
