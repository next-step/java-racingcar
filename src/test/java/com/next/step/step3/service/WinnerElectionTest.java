...
public class WinnerElectionTest {
    ...
    void electionWinners_우승자_한_명(Cars cars, List<String> expected) {
    ...
    void electionWinners_우승자_여러_명(Cars cars, List<String> expected) {
        List<String> result = WinnerElection.electionWinners(cars);
        assertThat(result).containsAnyElementsOf(expected);
    }
    ...
    static Stream<Arguments> provideOneWinnerCars() {
        return Stream.of(
                Arguments.of(new Cars(WinnerCarsFixture.한_명의_우승자_경우_CARS()), List.of("c"))
        );
    }

    static Stream<Arguments> provideManyWinnerCars() {
        return Stream.of(
                Arguments.of(new Cars(WinnerCarsFixture.모두_우승한_경우_CARS()), List.of("a", "b", "c")),
                Arguments.of(new Cars(WinnerCarsFixture.B_C가_우승한_경우_CARS()), List.of("b", "c"))
        );
    }
}
