...
class RacingGameApplicationTest {
    ...
    void racingGameTest(String nameInput, int tryCount){
        assertDoesNotThrow(() -> new RacingGameController().racingGame(nameInput, tryCount));
    }
    ...
    void racingGameWrongInputTest(String nameInput, int tryCount){
        assertThatThrownBy(()-> new RacingGameController().racingGame(nameInput, tryCount)).isInstanceOf(RuntimeException.class);
    }
}