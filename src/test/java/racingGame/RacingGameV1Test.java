...
import racingGame.racingRule.TestRacingRule;
...
public class RacingGameV1Test {
    ...
    private TestRacingRule testRacingRule = new TestRacingRule();

    @Test
    @DisplayName("차 대수 1, 시행 횟수 1를 입력하면 -를 list로 반환한다.")
    void racing_create_car_1_try_2() {
        //given
        GameInput testInput = new GameInput("1", "1");
        RacingGameV1 game_v1 = new RacingGameV1(testInput);

        Car testCar = new Car();
        testCar.move();
        //when
        List<Car> result = game_v1.start(testRacingRule);
        //then
    ...
    void racing_create_car_1_try2() {
        //given
        GameInput testInput = new GameInput("2", "2");
        RacingGameV1 game_v1 = new RacingGameV1(testInput);
        Car testCar = new Car();
