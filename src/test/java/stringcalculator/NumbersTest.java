...
class NumbersTest {
    ...
    void createStringCalculator(String text) {
        assertThatCode(() -> new NumberParser().parse(text, Delimiters.of(text)).sum())
    ...
    void createStringCalculatorWithInvalidText(String text) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new NumberParser().parse(text, Delimiters.of(text)).sum()).withMessage("구분자를 제외하고 숫자가 아닌 문자는 입력할 수 없습니다.");
    }
    ...
    void createStringCalculatorWithInvalidText2(String text) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new NumberParser().parse(text, Delimiters.of(text)).sum())
    ...
    void sumTest(String text) {
        int expectedSum = 6;
        final Numbers numbers = new NumberParser().parse(text, Delimiters.of(text));

        assertThat(numbers.sum()).isEqualTo(expectedSum);
    }
    ...
    void negativeNumbersNotAvailable(String text) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> new NumberParser().parse(text, Delimiters.of(text)))
    ...
    void nullOrEmptyStringIsZero(String text) {
        ...
        assertThat(new NumberParser().parse(text, Delimiters.of(text)).sum()).isEqualTo(expectedSum);
    }
