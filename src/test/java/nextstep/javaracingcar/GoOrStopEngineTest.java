...
class GoOrStopEngineTest {

    @DisplayName("랜덤에서 기준값 보다 크거나 같으면 1을 이동한다.")
    @ParameterizedTest
    @CsvSource({"5,4", "2,2", "9,4", "3,3"})
    public void spec01(final int randomValue, final int base) {
        final GoOrStopEngine goOrStopEngine = new GoOrStopEngine(() -> randomValue, base);
        assertThat(goOrStopEngine.work()).isEqualTo(Distance.ONE);
    }
    ...
    public void spec02(final int randomValue, final int base) {
        final GoOrStopEngine goOrStopEngine = new GoOrStopEngine(() -> randomValue, base);
        assertThat(goOrStopEngine.work()).isEqualTo(Distance.ZERO);
    }
}
