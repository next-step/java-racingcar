...
class RecordGroupTest {
    ...
    void findMaxDistance() {
        ...
        assertThat(new RecordGroup(Fixture.getAllRecords(records)).getMaxDistance()).isEqualTo(record3.getDistance());
    }
    ...
    void winnerPlayer(List<Record> records, int expected) {
        assertThat(new RecordGroup(Fixture.getAllRecords(records)).getWinners().split(",").length).isEqualTo(expected);
    }
