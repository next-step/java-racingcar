...
- 핵심 비지니스 로직을 가지는 객체를 domain 패키지에, UI 관련된 객체를 view 패키지에 구현할 것
- MVC 패턴 기반으로 리팩토링을 하고, view는 domain에 의존할 수 있지만, domain은 view에 의존하지 않도록 구현한다.
- 테스트 가능한 부분과 가능하지 않은 부분은 분리해서 가능한 부분만 단위 테스트를 진행한다.
