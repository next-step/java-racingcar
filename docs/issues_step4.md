...
- RacingGame 생성자
  ...
  ```java
   ...
   ```
    ...
    - 편의성을 위해 정적 팩토리 메서드를 추가: `new RacingGame(String[], int, Moveable)`
### TryCounts
- tryCounts 만큼 입력받은 함수를 반복하는 기능이 `TryCounts` 내부에 존재해도 되는가?
  - TryCounts 는 단순히 횟수를 나타나는 객체이므로 반복 기능을 넣는 것은 적합하지 않음
## 객체의 책임
- RacingGameResult
  ```java
