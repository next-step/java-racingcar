...
[X] InputView로부터 입력 받은 자동차 이름 목록을 쉼표로 구분하여, 이름 개수 만큼의 자동차 개체들을, 이름을 부여하여 생성 할 수 있다.

- [X]  입력 받은 이름 목록이 Null일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [X]  입력 받은 이름 목록이 Empty일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [X]  입력 받은 이름 목록을 쉼표로 구분할 수 없을 경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [X]  입력 받은 이름이 Null일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [X]  입력 받은 이름이 Empty일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [X]  입력 받은 이름이 5자를 초과 할 경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.

[X] InputView로부터 입력 받은 시도 회수를 통해, 해당 회수만큼 자동차 경주게임을 플레이 할 수 있다.
...
[X] 매 회 플레이가 끝나면, OutputView에, 현재 자동차 개체들의 진행상태를 반환 할 수 있다.

- [X]  한 회 플레이가 끝나면, Cars개체에게, '소유하고 있는 자동차 개체들의 이름과, 전진 거리 목록'를 반환하라는 메시지를 전달하여, 반환받은 리스트를
  OutputView에 전달한다.

[X] 게임을 완료 한 후, OutputView에, 우승한 자동차의 이름목록을 반환 할 수 있다.

- [X]  주어진 시도 회수 만큼의 플레이가 끝나면, Cars개체에게, '가장 멀리 전진한 자동차 개체들의 이름목록'를 반환하라는 메시지를 전달하여, 반환받은 리스트를
  OutputView에 전달한다.
...
[X] Client는, Cars 개체로부터, '소유하고 있는 자동차 개체들의 이름과, 현재 전진 거리 상황'을 반환할 수 있어야 한다.

### View

#### InputView

[X] 사용자로부터, 경주 할 자동차 이름 목록을 입력 받을 수 있다.
[X] 사용자로부터, 몇번의 전진을 시도 할 것인지 입력받을 수 있다.

#### OutputView

[X] 각 회수별, 각 자동차의 전진 실행 상태를, 자동차 이름 별로 출력할 수 있다.
[X] 플레이 후, 가장 멀리 전진한 자동차들의 이름 목록을 출력할 수 있다.
...
[X] MVC 패턴 기반으로 리팩토링해 view 패키지의 객체가 domain 패키지 객체에 의존할 수 있지만, domain 패키지의 객체는 view 패키지 객체에 의존하지 않도록
구현한다.
...
A: 케이스 바이 케이스겠지만, 이번 미션의 경우, 유저 입력으로 인한 IAE의 경우, 다시 유저 입력을 받는 것으로 리커버리 가능하다.
