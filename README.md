...
---
    요구사항 -> 단위테스트 내용 추출
    - 자동차의 position만큼 '-' 문자열로 변환하여 출력하여야 한다.
    - 자동차의 position은 입력된 값이 4이상 인 경우 증가하고, 4미만의 경우 position의 변경은 없다
---
### Done
- [x] 자동차 '-' 문자열 출력 테스트 : 자동차의 현재 position를 확인 후 '-' 문자열 변경 테스트를 한다.
- [x] 자동차 이동 테스트 : 자동차가 입력 받은 값이 4이상 일때만, position 증가를 테스트 한다.
---
# 1단계 - 학습 테스트 실습 (+)
