...
5. 참고문서: https://google.github.io/styleguide/javaguide.html 또는 https://myeonguni.tistory.com/1596
6. else 예약어를 쓰지 않는다.
...
1. `InputView`에서 String 값으로 자동차 이름을 입력 받는다.
    - 자동차 이름은 쉼표(,)를 기준으로 구분한다.
2. 자동차(RacingCar) 이름 기능 추가
    - 각 자동차에 이름을 부여할 수 있다.
    - 자동차 이름은 5자를 초과할 수 없다.
3. `ResultView`의 출력 과정(`print()`)에서 자동차의 이름을 출력한다.
    - 전진하는 자동차를 출력할 때 자동차 이름을 같이 출력한다.
4. `RacingRound`는 누가 우승했는지 알 수 있다.
    - 누가 우승했는지 알 수 있도록 일급 컬렉션에서 기능을 구현한다.
...
![domain1](doc/img/domain.PNG)

- 테스트 가능한 부분과 테스트하기 힘든 부분을 분리해 테스트 가능한 부분에 대해서만 단위 테스트를 진행한다.

![domain2](doc/img/domain2.PNG)
