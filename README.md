...
[ ] InputView로부터 입력 받은 자동차 이름 목록을 쉼표로 구분하여, 이름 개수 만큼의 자동차 개체들을, 이름을 부여하여 생성 할 수 있다.

- [ ]  입력 받은 이름 목록이 Null일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [ ]  입력 받은 이름 목록이 Empty일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [ ]  입력 받은 이름 목록을 쉼표로 구분할 수 없을 경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [ ]  입력 받은 이름이 Null일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [ ]  입력 받은 이름이 Empty일경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
- [ ]  입력 받은 이름이 5자를 초과 할 경우, IllegalArgumentException을 발생시켜, 프로그램을 종료한다.
...
- [ ]  한 회 플레이가 끝나면, Cars개체에게, '소유하고 있는 자동차 개체들의 이름과, 전진 거리 목록'를 반환하라는 메시지를 전달하여, 반환받은 리스트를
  OutputView에 전달한다.

[ ] 게임을 완료 한 후, OutputView에, 우승한 자동차의 이름목록을 반환 할 수 있다.

- [ ]  주어진 시도 회수 만큼의 플레이가 끝나면, Cars개체에게, '가장 멀리 전진한 자동차 개체들의 이름목록'를 반환하라는 메시지를 전달하여, 반환받은 리스트를
  OutputView에 전달한다.
