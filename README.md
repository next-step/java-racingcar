...
- 테스트하기 힘든 코드를 테스트 가능한 구조로 변경하기 위한 방법
    - 내부 API (Random)가 객체 그래프의 하위의 도메인 계층에 존재하면 테스트하기 힘들어지므로 `상위 계층에 종속`되어 전달할 수 있도록 수정 
    - 또는 `인터페이스`를 통해 `외부에서 주입`을 통한 구현

- 모든 원시값과 문자열을 포장
    - 원시 값과 문자열에 대한 예외 처리 로직을 객체 내에 담을 수 있다.
    - getter 메서드를 가능한 사용하지 않기
    - `값`으로 `비교하지 않고` `객체` 단위로 `비교`할 수 있도록 한다. (`동일성`, `동등성` 비교)
    - 값에 대한 `유효성 검사`를 하기 위해서 비즈니스 로직에서 해야 하는 문제를 해결할 수 있다.
    - 비즈니스 코드에서는 값을 변경하지 않고(`연산 코드`) 메시지를 전달(`메서드`)하여 `작업에 대한 처리`를 `위임`하는 방식으로 구현한다. 
    - 한번 값이 생성되면 수정할 수 없는 Immutable Object, 수정이 가능한 경우 mutable Object
        - Immutable Object 같은 경우 객체가 계속 생성되므로 성능 문제가 발생 할 수 있다.
        - mutable Object 같은 경우 값을 변경하고 this 를 반환하여 위 성능 문제를 해결할 수 있다.
    - `클래스`를 분리하여 `단일책임원칙`을 지킬 수 있다.
    
    - 유효성 검사
        - view 에서 유효성 검사를 하는 것은 필수
        - domain 에서 값이 변경하는 로직이 있는 경우 유효성 검사가 필요

- 일급 컬렉션
    - 컬렉션을 필드로 포함하는 클래스를 작성하여 관련 행위에 대한 코드를 갖는다.
    
## 5. 자동차 경주(리펙토링)
- 객체지향 설계로 구현했는지 확인하기
- 구현 시에 TDD로 구현이 가능함에도 이전 습관으로 구현했는지 확인하기
- 테스트 가능하도록 수정해보았는지 확인하기
